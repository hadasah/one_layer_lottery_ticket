...
def change_model_mode(model, mode="training"):
    for name, module in model.named_modules():
        try:
            ...
            pass
    print(f'Successfully change to model to {mode} mode.')
...
def main(cfg: DictConfig) -> None:
    ...
    logger.info(
        "num. model params: {} (num. trained: {}, num. weight: {}, num. mask: {})".format(
            sum(p.numel() for p in model.parameters()),
            sum(p.numel() for p in model.parameters() if p.requires_grad),
            sum(p.numel() for n, p in model.named_parameters() if "score" not in n),
            sum(p.numel() for n, p in model.named_parameters() if "score" in n),
        )
    )
    ...
    # for name, param in trainer.get_model().named_parameters():

    #     if "score" not in name:
    #         print(100 * '*')
    #         print(f"This {name} loads weight from pre-trained model")
    #         print(param.data.cpu().view(-1)[0])
...
def train(
    ...
    change_model_mode(trainer._model, mode="train")
...
def validate(
    ...
    # NOTE: change the mode 
    change_model_mode(trainer._model, mode="eval")
