...
DEFAULT_DIR_PATH = '/gscratch/zlab/margsli/gitfiles/one_layer_lottery_ticket'
BASH_IF_CLAUSE = """
...
cd {NEW_DIR_PATH}
export PYTHONPATH={SAVE_ROOT}/one_layer_lottery_ticket:$PYTHONPATH
{python_cmd} &
...
def run_grid(
    ...
    add_name -- (str) "beginning" or "end" or None, indicating whether to
    ...
    if grid.get('fixed_args'):
        new_jobs = []
        for job in all_jobs:
            new_cmd = ' '.join((job.cmd, grid['fixed_args']))
            new_name = job.name
            new_jobs.append(Job(cmd=new_cmd, name=new_name))
        all_jobs = new_jobs
    ...
    if output_dir_name:
        new_jobs = []
        for job in all_jobs:
            new_cmd = '{} {} {}/{}'.format(job.cmd, output_dir_name, SAVE_ROOT, job.name)
            new_name = job.name
            new_jobs.append(Job(cmd=new_cmd, name=new_name))
        all_jobs = new_jobs
    ...
    if copy_env:
        bash('mkdir -p ' + os.path.join(SAVE_ROOT, 'one_layer_lottery_ticket'))
        to_copy = []
        to_copy += copy_dirs
        for c in to_copy:
            ...
            if len(c_head) > 1:
                bash('mkdir {SAVE_ROOT}/one_layer_lottery_ticket/{c_head}'.format(**locals()))
            bash('cp -r {DIR_PATH}/{c} {SAVE_ROOT}/one_layer_lottery_ticket/{c}'.format(**locals()))
        NEW_DIR_PATH = '{SAVE_ROOT}/one_layer_lottery_ticket'.format(**locals())
    else:
...
def save_root(SWEEP_NAME, unixname, top_level_experiments_folder):
    """Return root folder for saving model files, stdout, stderr, etc."""
    DATE = bash('date +"%Y%m%d"')
    SAVE_ROOT = os.path.join(top_level_experiments_folder, unixname, DATE, SWEEP_NAME)
    return SAVE_ROOT

def log_root(SWEEP_NAME, unixname, top_level_experiments_folder):
    """Return root folder for saving tensorboard logs"""
    DATE = bash('date +"%Y%m%d"')
    LOG_ROOT = os.path.join(top_level_experiments_folder, unixname, DATE, SWEEP_NAME)
    return LOG_ROOT
...
def submit_array_jobs(
    ...
    if dependencies:
        SBATCH_EXTRAS.append('#SBATCH --dependency="{}"'.format(','.join(['afterok:' + d for d in dependencies])))
